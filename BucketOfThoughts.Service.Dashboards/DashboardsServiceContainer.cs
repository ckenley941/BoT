...
namespace BucketOfThoughts.Service.Dashboards
{
    public class DashboardsServiceContainer
    {
        ...
        public DashboardsServiceContainer(IDistributedCache cache, LanguageDbContext languageDbContext, ICrudRepository<Thought> thoughtsRepository)
        {
            _wordsService = new(() => { return new WordsService(languageDbContext); });
            _thoughtsService = new(() => { return new ThoughtsService(thoughtsRepository, cache); });
        }
